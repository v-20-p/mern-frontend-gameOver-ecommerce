...
export const sendChat = createAsyncThunk('chats/sendChat', async (message: string) => {
  try {
    ...
    return [
      { sender: 'user', content: message, _id: response.data.botMessage._id + '2wwqx' },
...
const chatSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addMatcher(isPending(sendChat, fetchChats), (state) => {
        state.isLoading = true
      })
      .addMatcher(isRejected(sendChat, fetchChats), (state, action) => {
